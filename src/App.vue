...
async function handleDeepLink(urlStr: string) {
  try {
    ...
    const parsedUrl = new URL(urlStr.replace('com.brandwala.thriftapp://', 'http://localhost/'))
    ...
    if (!sessionSet && searchParams.has('access_token') && searchParams.has('refresh_token')) {
    ...
    if (sessionSet) {
      ...
      await router.replace({ path: '/auth/callback', query: { scope: 'app', tenant_slug: tenantSlug } })
    } else {
      console.warn('[handleDeepLink] No session set. URL had no recognised token params.')
    }
  } catch (err) {
