        ...
        <div class="col-12 col-md-5 q-gutter-y-md">
          ...
          <q-card class="app-card">
            <q-card-section>
              ...
              <div
                v-if="tempBarcode"
                class="app-capture-zone q-pa-md text-center relative-position"
              >
                <div
                  class="text-h6 text-weight-medium text-grey-9 letter-space-wide q-py-xs"
                >
                  {{ tempBarcode }}
                </div>
                <div
              ...
              <div
                v-else
                class="q-py-sm q-gutter-y-sm app-capture-zone app-capture-zone--required"
              >
                <q-btn
                  color="primary"
                  unelevated
          ...
          <q-card class="app-card">
            <q-card-section>
              ...
              <div
                v-if="previewImageUrl"
                class="app-capture-zone q-pa-sm text-center relative-position"
              >
                <q-img
                  :src="previewImageUrl"
              ...
              <div
                v-else
                class="text-center q-py-lg app-capture-zone app-capture-zone--required"
              >
                <q-btn
                  color="primary"
                  unelevated
                ...
                <div class="text-caption text-grey-6 q-mt-xs"
                  >Capture physical item appearance</div
                >
              </div>
            </q-card-section>
          </q-card>
          ...
          <q-card class="app-card">
            ...
            <q-card-section class="q-gutter-y-md">
              <div class="app-required-hint">
                <q-icon name="info" />
                Fields marked with
                <span class="app-field-label__mark">*</span> are required
              </div>
              <q-form ref="stockFormRef" class="q-gutter-y-md">
                <div>
                  <label
                    class="app-field-label app-field-label--required block q-mb-xs"
                  >
                  ...
                  <q-input
                    ...
                    :class="{
                      'app-required-input--pending': !form.brand_name?.trim()
                    }"
                ...
                <div>
                  ...
                  <q-input
                    ...
                    class="app-required-input"
                    :class="{
                      'app-required-input--pending': !form.size?.trim()
                    }"
                    :rules="[val => !!val?.trim() || 'Required']"
                  />
                </div>
                ...
                <div>
                  ...
                  >
                    Product Weight (g)
                    <span class="app-field-label__mark">*</span>
                  </label>
                  <q-input
                    ...
                    class="app-required-input"
                    :class="{
                      'app-required-input--pending':
                        form.product_weight == null || form.product_weight <= 0
                    }"
                ...
                <div>
                  <label
                    class="text-caption text-weight-bold text-grey-7 block q-mb-xs"
                    >Note / Remarks</label
                  >
                  <q-input
                    v-model="form.note"
...
const userEmail = computed(
  () => authStore.user?.email || "app-user@brandwala.com"
);
...
const barcodeStatusLabel = computed(() => {
  if (barcodeAvailability.value === "available")
    return "Available — ready to register";
  if (barcodeAvailability.value === "used")
    return "Already used — scan another barcode";
  if (barcodeAvailability.value === "not_found")
    return "Not in catalog — scan another barcode";
...
const applyScannedBarcode = async (barcodeVal: string) => {
  if (!tenantId.value) {
    $q.notify({
      type: "negative",
      message: "Workspace is missing. Go back and select one."
    });
    return;
  }
  ...
  try {
    const result = await validateBarcodeForRegistration(
      tenantId.value,
      barcodeVal
    );
    ...
    barcodeAvailability.value = null;
    const message =
      err instanceof Error ? err.message : "Could not validate barcode";
...
const startBarcodeScan = async () => {
  try {
    ...
    const message =
      err instanceof Error ? err.message : "Error scanning barcode";
    $q.notify({
      type: "warning",
...
const startCameraCapture = async () => {
  if (Capacitor.isNativePlatform()) {
    try {
      ...
      $q.notify({
        type: "negative",
        message: `${message}. Falling back to file upload.`
      });
...
const submitStock = async () => {
  ...
  if (!tempBarcode.value) {
    $q.notify({
      type: "negative",
      message: "Please scan or input a barcode first"
    });
    return;
  }
  ...
  if (barcodeAvailability.value !== "available") {
    $q.notify({
      type: "negative",
      message: "Please scan an available barcode before registering"
    });
    return;
  }
  ...
  if (!webBlob.value) {
    $q.notify({
      type: "negative",
      message: "Please take or choose a product photo first"
    });
    return;
  }
