export function buildBarcodeCandidates(
  tenantId: number,
  raw: string
): string[] {
  ...
  if (fourPart && fourPart[1] && fourPart[2] && fourPart[3] && fourPart[4]) {
    candidates.add(
      `${fourPart[1]}-${fourPart[2]}-${fourPart[3]}-${fourPart[4].padStart(6, "0")}`
    );
  }
  ...
  if (threePart && threePart[1] && threePart[2] && threePart[3]) {
    candidates.add(
      `${tenantPrefix}-${threePart[1]}-${threePart[2]}-${threePart[3].padStart(6, "0")}`
    );
