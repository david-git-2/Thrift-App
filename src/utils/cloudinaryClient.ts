import { invokeEdgeFunctionJson } from "./edgeFunctionClient";
...
export function getThriftCloudinaryRootFolder(): string {
  const configured = import.meta.env.VITE_CLOUDINARY_THRIFT_FOLDER as
    | string
    | undefined;
  const root = configured?.trim() || "thrift-inventory-images";
...
export function buildThriftStockImageFileName(barcode: string): string {
  ...
  return base.toLowerCase().endsWith(".jpg") ||
...
export function buildThriftShipmentCloudinarySubfolder(
  shipmentId: number
): string {
...
export async function uploadToCloudinary(
  ...
  const shipmentFolder =
    options?.shipmentFolder?.trim() ||
    options?.assetFolder?.trim() ||
    folder?.trim();
  if (shipmentFolder) {
  ...
  return {
    ...
    ...(resData.delete_token
      ? { deleteToken: resData.delete_token as string }
      : {})
  };
}
...
export async function deleteCloudinaryByToken(
  ...
  try {
    ...
    if (!response.ok) {
      console.warn(
        "Failed to delete image using delete token:",
        await response.text()
      );
    }
  } catch (err) {
...
export async function deleteCloudinaryImage(imageUrl: string): Promise<void> {
  ...
  try {
    await invokeEdgeFunctionJson("cloudinary-delete", { imageUrl });
  } catch (err) {
    const message =
      err instanceof Error ? err.message : "Cloudinary delete failed";
    console.warn("Failed to delete Cloudinary image:", message);
  }
}
