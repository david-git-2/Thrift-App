...
export async function updateThriftStock(params: UpdateThriftStockParams): Promise<void> {
  ...
  const { error: pricingError } = await supabase.from('thrift_pricings').upsert(
    {
      ...
      cost_of_goods_sold: params.pricing.cost_of_goods_sold,
      target_price: params.pricing.target_price,
      listed_price: params.pricing.listed_price,
      extra_expense_cost: params.pricing.extra_expense_cost ?? 0,
      inserted_by: insertedBy,
    },
    { onConflict: 'stock_id' },
  )
