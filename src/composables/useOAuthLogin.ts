...
export function useOAuthLogin() {
  ...
  const handleEmailPasswordLogin = async (email: string, password: string) => {
    ...
    try {
      ...
      if (error) throw error
      if (data.session) {
        return await bootstrapUserSession(data.session.user.email ?? email, data.session)
      }
      return false
    } catch (err: any) {
  ...
  const bootstrapUserSession = async (userEmail: string, session: any) => {
    ...
    const { data: tenantsData, error: tenantsError } = await supabase.rpc('list_tenants_by_membership', {
      p_email: formattedEmail,
      p_role: null,
      p_tenant_id: null,
    })
    ...
    const { data: bootstrapData, error: bootstrapError } = await supabase.rpc('get_app_bootstrap_context', {
      p_email: formattedEmail,
      p_tenant_id: targetTenant.id,
      p_membership_id: result.member_tenant_id === targetTenant.id ? result.member_id : null,
    })
