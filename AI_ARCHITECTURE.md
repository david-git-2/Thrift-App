...
| `thrift_pricings` | COGS, target, listed, `extra_expense_cost` |
...
- **Purchase currency** (from shipment `purchase_currency_id`): `origin_purchase_price`, `extra_origin_purchase_expense`
- **Cost currency** (from shipment `cost_currency_id`): `cost_of_goods_sold`, `extra_expense_cost`, `target_price`, `listed_price`
